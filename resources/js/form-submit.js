...
export const collectFields = (form) => {
    const fields = {};
...
export const showFormErrors = (form, errors, controlSelector = '.form-control') => {
    form.querySelectorAll('[data-field-error]').forEach((node) => {
        node.hidden = true;
        node.textContent = '';
    });

    form.querySelectorAll(controlSelector).forEach((el) => {
...
export const bindFormSubmit = (form) => {
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 422 && data.errors) {
                    showFormErrors(form, data.errors, form.dataset.controlSelector || '.form-control');
                }
            ...
            form.reset();
            showFormErrors(form, {}, form.dataset.controlSelector || '.form-control');
            ...
            form.dispatchEvent(new CustomEvent('form:success', { bubbles: true, detail: data }));
        } catch {
            if (messageNode) {
...
export const initFormSubmit = (selector = '[data-ajax-form]') => {
    document.querySelectorAll(selector).forEach(bindFormSubmit);
};
